  ...
  try {
    const response = await callGemini([
      ...
      },
      {
        role: "system",
        content:
          "Return either 'react' or 'next' based on the user's request. If they mention Next.js, return 'next'. Otherwise, return 'react' as default. Only return a single word.",
      },
    ]);

    const answer = response.trim().toLowerCase();
    return answer === "next" ? "next" : "react";
  } catch (error) {
  ...
  try {
    ...
    if (url) {
      ...
      if (scrapedData.length > 10) {
        console.warn(
          `Large website detected: ${scrapedData.length} pages. Limiting to first 10 pages for token efficiency.`,
        );
...
router.post("/clone-chat", async (req, res) => {
  try {
    const { prompts, uiPrompts, template, scrapedData } = req.body;
    ...
    // Final instruction
    conversationText +=
      "User: Generate complete code implementation for this website clone using the specified template. " +
